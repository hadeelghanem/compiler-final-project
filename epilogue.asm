...
;;; fill in for final project!
        ...
        ; Get the function argument (first arg)
        mov r12, qword [rsp + 8 * 3]
        assert_closure(r12)
        
        ; Get the list argument (second arg)
        lea r10, [rsp + 8 * 4]
        mov r11, qword [r10]
        ...
        mov rsi, r11            ; rsi points to current list node
.count_list_loop:
        cmp rsi, sob_nil        ; Check if we reached end of list
        je .count_list_done
        assert_pair(rsi)        ; Ensure current element is a pair
        inc rcx                 ; Increment counter
        mov rsi, SOB_PAIR_CDR(rsi)  ; Move to next element
        jmp .count_list_loop
        
.count_list_done:
        ; Adjust stack for new frame
        lea rbx, [8 * (rcx - 2)]
        sub rsp, rbx
        mov rdi, rsp
        cld
        ...
        mov rax, r9             ; Return address
        stosq
        mov rax, SOB_CLOSURE_ENV(r12)  ; Environment
        stosq
        mov rax, rcx            ; Argument count
        stosq
        
.copy_args_loop:
        test rcx, rcx           ; Check if we're done
        jz .copy_args_done
        mov rax, SOB_PAIR_CAR(r11)  ; Get current argument
        stosq
        mov r11, SOB_PAIR_CDR(r11)  ; Move to next pair
        dec rcx
        jmp .copy_args_loop
        
.copy_args_done:
        ; Verify stack integrity
        sub rdi, 8              ; Adjust rdi to match expected position
        cmp r10, rdi
        jne .stack_error
